...
void MapScene::updateBombExplode(int x, int y, int power)
{
	...
	if (Map[x + lenX + 1][y] == 3 && lenX < power) {     //引爆炸弹
		bomb[x + lenX + 1][y] = 51;
	}
...
{
	int lenX = 0, len_X = 0, lenY = 0, len_Y = 0;
	while (Map[x + lenX + 1][y] <= 0 && lenX < power) {
		lenX++;
	...
	while (Map[x - len_X - 1][y] <= 0 && len_X < power) {
		len_X++;
	...
	while (Map[x][y + lenY + 1] <= 0 && lenY < power) {
		lenY++;
	...
	while (Map[x][y - len_Y - 1] <= 0 && len_Y < power) {
		len_Y++;
